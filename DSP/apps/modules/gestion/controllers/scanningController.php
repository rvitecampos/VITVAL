...
class scanningController extends AppController {
   ...
   public function get_list($p){
    ...
    public function set_scanner_file_one_to_one($p){
        ...
        if(isset($records)){
            if (!file_exists(PATH.'public_html/scanning/'.$p['con'])) {
                mkdir(PATH.'public_html/scanning/'.$p['con'], 0777, true);
            }
            foreach($records as $record){
                ...
                if (file_exists(PATH.'public_html/contenedor/'.USR_ID.'/'.$file)){
                    ...
                    if($rs['status']=='OK'){
                        rename(PATH.'public_html/contenedor/'.USR_ID.'/'.$file, PATH.'public_html'.$p['vp_path'].$p['vp_lote_nombre'].'-'.$file);
                        try{
